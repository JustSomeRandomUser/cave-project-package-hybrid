	...
	namespace Frustum
	{
		public class ProjectionMatrix : MonoBehaviour
		{
			...
			private void CalculateBimberMatrix()
			{
				...
				Debug.Log(bimberMatrix);
			}

			//private Matrix4x4 GetBimberMatrixFormHomography(Matrix<double> homography)
			private Matrix4x4 GetBimberMatrixFormHomography(float[,] homography)
			{
				/*
				...
				*/
				...
				return Projection.Bimber(homography);
			}

			//private Matrix<double> GetHomography(Vector2[] src, Vector2[] dest)
			private float[,] GetHomography(Vector2[] src, Vector2[] dest)
			{
				/*
				...
				*/
				...
				return Homography.Find(src, dest);
			}
			...
			private Matrix4x4 PerspectiveOffCenter(NearPlane np)
			{
				...
				*/
				...
				return Projection.Holographic(new float[3,2]{
					{np.left, np.right},
					{np.bottom, np.top},
					{np.near, np.far}
				});
			...
			private void CreatePixelMask(Quad frame)
			{
				...
				PixelMask mask = new PixelMask(frame.points);
				mask.Apply(texture, Color.black);
				Debug.Log(bimberMatrix);
			...
			private void LoadHomographyMatrix()
			{
				...
				try
				{
					...
					{
						CultureInfo ci = (CultureInfo)CultureInfo.CurrentCulture.Clone(); //WORKAROUND
						ci.NumberFormat.CurrencyDecimalSeparator = ".";

						for (int i = 0; i < 16; i++)
						{
							bimberMatrix[i] = float.Parse(reader.ReadLine(), NumberStyles.Any, ci);
						}
						...
						for (int i = 0; i < 4; i++)
						{
							cornorPoints[i].x = float.Parse(reader.ReadLine(), NumberStyles.Any, ci);
							cornorPoints[i].y = float.Parse(reader.ReadLine(), NumberStyles.Any, ci);
